...
with sql.connect(
    ...
    with connection.cursor() as cursor:
        ...
        volume_table = table_name(catalog, schema, volume)

        cursor.execute(
            f"CREATE VOLUME IF NOT EXISTS {volume_table}"
        )
...
uploads = {
    "raw_payments.csv": data_dir / "raw_payments.csv",
    "processed_payments.csv": data_dir / "processed_payments.csv",
    "hourly_revenue.csv": data_dir / "hourly_revenue.csv",
}
...
for remote_name, local_path in uploads.items():
    ...
    remote_path = f"{volume_path}/{remote_name}"
...
with sql.connect(
    ...
    with connection.cursor() as cursor:
        # Remove staging metadata left by an earlier failed run.
        for stage_name in (
            ...
            cursor.execute(
                ...
                f"{table_name(catalog, schema, stage_name)}"
            )
        ...
        cursor.execute(
            ...
            SELECT
                ...
                payment_method,
                currency,
                CAST(payment_count AS BIGINT) AS payment_count,
        ...
        cursor.execute(
            f"SELECT COUNT(*) FROM {hourly_table}"
        )
        ...
        print(f"Processed table rows: {processed_count}")
        print(f"Hourly table rows: {hourly_count}")
        ...
        for method, count, revenue_cents in cursor.fetchall():
            print(
                f"{method}: {count} payments, "
                f"SGD {revenue_cents / 100:,.2f}"
            )
